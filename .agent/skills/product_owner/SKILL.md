...
4. **Sincronização com o Jira (OBRIGATÓRIO):** Sempre que você formular ou aprovar uma História de Usuário nova, você DEVE enviá-la ao Jira rodando: `node scripts/jira.js create-story "US.XXX.XX - Tít" "Descrição" CHAVE-DO-EPICO`. O Épico é **OBRIGATÓRIO**. Se o Épico não existir, crie-o primeiro rodando `node scripts/jira.js create-epic "Nome" "Desc"`.
